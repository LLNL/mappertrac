...
def maskseeds(root_dir,input_dir,output_dir,low_threshold,high_threshold,high_threshold_thalamus,force=False):
    ...
    run('fslmaths', " %s -thr %f -uthr %f -bin tmp.nii.gz" % (join(root_dir,"FA.nii.gz"),low_threshold,high_threshold))
    run('fslmaths', " %s -thr %f -uthr %f -bin tmp_thalamus.nii.gz" % (join(root_dir,"FA.nii.gz"),low_threshold,high_threshold_thalamus))
    ...
    for seed in glob(join(input_dir,"*s2fa.nii.gz")):
        ...
        if force or not exists(join(output_dir,split(seed)[1])):           
            if region == "thalamus":
                run('fslmaths', " %s -mas tmp_thalamus.nii.gz %s" % (seed, join(output_dir,split(seed)[1])))
            else:
                run('fslmaths', " %s -mas tmp.nii.gz %s" % (seed, join(output_dir,split(seed)[1])))
...
def saveallvoxels(root_dir,cortical_dir,subcortical_dir,output_name,force):
    ...
    if force or not exists(join(root_dir,"cort.nii.gz")):
        run("find_the_biggest", " %s %s" % (all_vols,join(root_dir,"cort.nii.gz")))
    ...
    if force or not exists(join(root_dir,"subcort.nii.gz")):
        run("find_the_biggest", " %s %s" % (all_vols,join(root_dir,"subcort.nii.gz")))
    
    if force or not exists(output_name):
        run('fslmaths', " %s -add %s %s " % (join(root_dir,"cort.nii.gz"),join(root_dir,"subcort.nii.gz"),output_name))
        run('fslmaths', " %s -bin %s" % (output_name,output_name))
