...
+- <SUBJECT NAME>/
   ...
   +- connectome_oneway.dot           # Oneway connectome in list form. 
                                        Each edge has two numbers: the first is number of successful streamlines and the second is average streamline density in the target region.
...
| edge_list           | lists/list_edges_reduced.txt    | Text file list of edges for steps s3_probtrackx and s4_edi |
...
| s4_cores_per_task   | 1                               | Number of cores to assign each task for step s4_edi |
| s5_cores_per_task   | 1                               | Number of cores to assign each task for step s5_render |
...
| s5_nodes            | [[floor(0.1 * num_subjects)]]   | Node count for step s5 |
...
| s5_cores            | [[core count on head node]]     | Cores per node for step s5 |
