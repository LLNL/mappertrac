...
void join_Parts(NEWIMAGE::volume<float> mask, string name_in, string name_out, string subjdir, int nvox, int nsamples, int nParts, float max, float min){
		...
		fprintf(stderr,"name_in %s; name_out %s\n",name_in.c_str(), name_out.c_str());
...
int main(int argc, char *argv[])
{
	...
	fprintf(stderr,"Time CP 0 %d\n",time(NULL));
    	NEWIMAGE::volume<float> mask;
    	read_volume(mask,opts.maskfile.value());

        fprintf(stderr,"Time CP 1 %d\n",time(NULL));
	...
	join_Parts(mask,"mean_S0samples","mean_S0samples",subjdir, nvox, 1, nParts, -10, 0);
        ...
        fprintf(stderr,"Time CP 2 %d\n",time(NULL));


	//for(int f=0;f<opts.nfibres.value();f++){
	int type;
	int f;
#pragma omp parallel private(type,f) num_threads(3*opts.nfibres.value()) 
	{
	  type = omp_get_thread_num()%3;
	  f = omp_get_thread_num() / opts.nfibres.value();
	  switch (type) { 
	  case 0:
	  ...
	  case 2: 
	    ...
	    break;
	  default:
	    fprintf(stderr,"ERROR in merge_parts_gpu.cc");
	    break;
	  }
	}
	//}
	    ...
	    //fprintf(stderr,"Time CP 3 %d\n",time(NULL));

		
		
	fprintf(stderr,"Time CP 4 %d\n",time(NULL));
