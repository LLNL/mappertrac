...
if force or not exists(join(output_dir,"mri/orig/001.mgz")):
    system("time " + join(fs_dir,'bin/mri_convert') + " %s %s" % (abspath(argv[2]),join(output_dir,"mri/orig/001.mgz")))
...
if force or not exists(join(output_dir,abspath(argv[2]))):
    system("time " + join(fs_dir,'bin/mri_convert') + " %s %s " % (join(output_dir,"mri","brain.mgz"),abspath(argv[2])))

    
if force or not exists(join(output_dir,"FA2T1.mat")):
    system("time " + join(fsl,'bin/flirt') + " -in %s -ref %s -omat %s" % (join(output_dir,"FA.nii.gz"),abspath(argv[2]),join(output_dir,"FA2T1.mat")))
    
if force or not exists(join(output_dir,"T12FA.mat")):
    system("time " + join(fsl,'bin/convert_xfm') + " -omat %s -inverse %s" % (join(output_dir,"T12FA.mat"),join(output_dir,"FA2T1.mat")))
...
if force or not exists(join(output_dir,cortical_dir,"rh.temporalpole.label")):
    system("time " + join(fs_dir,"bin",'mri_annotation2label') + " --subject %s --hemi rh --annotation aparc --outdir %s" % (subject,join(output_dir,cortical_dir)))

if force or not exists(join(output_dir,cortical_dir,"lh.temporalpole.label")):
    system("time " + join(fs_dir,"bin",'mri_annotation2label') + " --subject %s --hemi lh --annotation aparc --outdir %s" % (subject,join(output_dir,cortical_dir)))
...
for label in glob(join(output_dir,cortical_dir,"*.label")):
    ...
    if force or not exists(join(output_dir,vol_dir,vol_name)):
        system("time " + join(fs_dir,"bin",'mri_label2vol') + " --label %s --temp %s --identity --o %s" % (label,T1,join(output_dir,vol_dir,vol_name)))
    
# make_subcortical_vols
if force or not exists(join(output_dir,"aseg.nii.gz")):
    system("time " + join(fs_dir,'bin/mri_convert') + " %s %s" % (join(output_dir,"mri","aseg.mgz"),join(output_dir,"aseg.nii.gz")))
...
if force or not exists(join(output_dir,sub_vol_dir,"lh_acumbens.nii.gz")):    
    ...
    for line in open(indices,"r").readlines():
        ...
        system("time " + join(fsl,'bin/fslmaths') + " %s -uthr %s -thr %s -bin %s" % (join(output_dir,"aseg.nii.gz"),num,num,
...
if force or not exists(join(output_dir,vol_dir_out,"rh.bankssts_s2fa.nii.gz")):     
    for volume in glob(join(output_dir,vol_dir,"*.nii.gz")):
        ...
        system("time " + join(fsl,'bin/flirt') + " -in %s -ref %s -out %s  -applyxfm -init %s" % (volume,join(output_dir,"FA.nii.gz"),
                                                                                    out_vol,join(output_dir,"T12FA.mat")))
        system("time " + join(fsl,'bin/fslmaths') + " %s -thr %s -bin %s " % (out_vol,threshold,out_vol))
...
if exists(join(output_dir,"bs.nii.gz")):
    ...
    system("time " + join(fsl,'bin/fslmaths' + " %s -uthr .15 %s" % (join(output_dir,"FA.nii.gz"),join(output_dir,"terminationmask.nii.gz"))))

    system("time " + join(fsl,'bin/fslmaths' + " %s -add %s %s" % (join(output_dir,"terminationmask.nii.gz"),join(output_dir,"bs.nii.gz"),join(output_dir,"terminationmask.nii.gz"))))
    system("time " + join(fsl,'bin/fslmaths' + " %s -bin %s" % (join(output_dir,"terminationmask.nii.gz"),join(output_dir,"terminationmask.nii.gz"))))
    system("time " + join(fsl,'bin/fslmaths' + " %s -mul %s %s" % (join(output_dir,"terminationmask.nii.gz"),join(output_dir,"allvoxelscortsubcort.nii.gz"),join(output_dir,"intersection.nii.gz"))))
    system("time " + join(fsl,'bin/fslmaths' + " %s -sub %s %s" % (join(output_dir,"terminationmask.nii.gz"),join(output_dir,"intersection.nii.gz"),join(output_dir,"terminationmask.nii.gz"))))
      
    system("time " + join(fsl,'bin/fslmaths' + " %s -add %s -add %s %s" % (join(output_dir,"bs.nii.gz"),
