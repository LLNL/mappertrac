...
from subscripts.utilities import run,smart_remove,smart_mkdir,is_float,write_output
...
def s3_1_probtrackx(sdir, a, b, stdout):
    ...
    if not exists(a_file) or not exists(b_file):
        write_output(stdout, "Error: Both Freesurfer regions must exist: {} and {}".format(a_file, b_file))
        return
    smart_remove(a_to_b_file)
    smart_mkdir(pbtk_dir)
    write_output(stdout, "Running subproc: {}".format(a_to_b))
...
def s3_2_edi_consensus(sdir, a, b, stdout, inputs=[]):
    ...
    if not is_float(amax):
        write_output(stdout, "Error: fslstats on {} returns invalid value {}".format(a_to_b_file, amax))
        return
    amax = int(float(amax))
    bmax = run("fslstats {} -R | cut -f 2 -d \" \" ".format(b_to_a_file), print_output=False, working_dir=pbtk_dir).strip()
    if not is_float(bmax):
        write_output(stdout, "Error: fslstats on {} returns invalid value {}".format(b_to_a_file, bmax))
        return
    bmax = int(float(bmax))
...
def s3_3_edi_combine(sdir, consensus_edges, stdout, inputs=[]):
    ...
    for a_to_b in consensus_edges:
        ...
        if not exists(consensus):
            write_output(stdout,"{} has been thresholded. See {} for details".format(a_to_b, join(pbtk_dir, "zerosl.txt")))
            continue
        if not exists(total):
