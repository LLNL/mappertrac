...
def s3_1_start(params, inputs=[]):
    from subscripts.utilities import run,smart_remove,smart_mkdir,write,record_start,add_binary_vol
    ...
    for vol in volumes:
        vol_file = join(EDI_allvols, vol + ".nii.gz")
...
@python_app(executors=['s3'], cache=True)
def s3_2_probtrackx(params, vol, inputs=[]):
    import time
    ...
    for vol2 in volumes:
        if vol != vol2:
            write(waypoints, join(EDI_allvols, vol2 + ".nii.gz"), params)
...
def setup_s3(params, inputs):
    ...
    s3_2_futures = []
    for vol in volumes:
        s3_2_futures.append(s3_2_probtrackx(params, vol, inputs=[s3_1_future]))
