...
parser.add_argument('script', choices=['s1','s2a','s2b','s2b_gpu','s3','s4'], type=str.lower, default='s1', help='Script to run across subjects')
...
# We create two executors: local and batch.
...
# Use only a single Slurm executor for now, to prevent requesting unnecessary resources
if s1:
...
if s1:
    @python_app(executors=['batch','local'])
    def s1_1_split_timesteps(input_dir, sdir, force):
    ...
    @python_app(executors=['batch','local'])
    def s1_2_timestep_process(sdir, step, force, inputs=[]):
    ...
    @python_app(executors=['batch','local'])
    def s1_3_dti_fit(input_dir, sdir, force, inputs=[]):
    ...
    @python_app(executors=['batch','local'])
    def s2b_2_freesurfer_postproc(sdir, force, inputs=[]):
    ...
    for input_dir in f.readlines():
        ...
        if s1:
            ...
            if not isInteger(num_timesteps):
                ...
                continue
            s1_1_future = s1_1_split_timesteps(input_dir, sdir, args.force)
            s1_2_futures = []
            for i in range(int(num_timesteps)):
                s1_2_future = s1_2_timestep_process(sdir, i, args.force, inputs=[s1_1_future])
                s1_2_futures.append(s1_2_future)
            s1_3_future = s1_3_dti_fit(input_dir, sdir, args.force, inputs=s1_2_futures)
            jobs.append(s1_3_future)
        elif s2a:
            s2a_future = s2a_bedpostx(sdir, args.force)
            jobs.append(s2a_future)
        elif s2b or s2b_gpu:
            s2b_1_future = s2b_1_freesurfer(sdir, args.force, s2b_gpu)
